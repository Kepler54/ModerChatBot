...
def handlers_register(dp: Dispatcher):
    ...
    @dp.message_handler(ReplyFilter(), lambda message: message.text.lower() == "кубик")
    async def dice(message: types.Message):
        await message.answer_dice()

    @dp.message_handler(ReplyFilter())
    ...
    @dp.message_handler()
    async def reaction_and_filter(message: types.Message):
        for i in sf.reaction_reading():
            if i in message.text.lower():
